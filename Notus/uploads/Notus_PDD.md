...
左侧文章区直接复用 WYSIWYG 编辑器和工具栏，可边看边改，也可一键收起。问答区顶部支持“参考来源”设置：自动匹配或手动指定文章。InputBar：模型 Select(sm，上拉展开) + TextArea(flex:1) + 发送按钮(40x40 bg:--accent radius-full)；`v1` 不展示 `+` 附件入口。
...
**AIPanel：** 顶部风格来源区域（自动匹配 / 手动指定，手动模式可动态添加参考文章） + 对话区（+ OperationPreview 嵌套） + InputBar（`@b2` 引用块，`@` 触发 autocomplete，模型选择框上拉展开）；`v1` 不展示 `+` 附件入口。
...
**状态：** 加载中(Skeleton) / 模型未就绪(整页浅遮罩 + 锁态卡片) / AI 处理中(脉冲 + StreamingText) / 大纲生成中(Block 骨架依次渐入)。
